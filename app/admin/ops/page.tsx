...
import { ArrowLeft, Check, Clock3, ExternalLink, MessageSquare, Save, ShieldCheck } from 'lucide-react'
...
export default function DeliveryOpsPage() {
  ...
  async function save() {
    ...
    try {
      ...
      setNotice('Project delivery state saved.'); await load(); const fresh = projects.find(p => p.id === selected.id); if (fresh) await openProject({ ...fresh, delivery_status: deliveryStatus, client_message: message })
  ...
  if (selected) return <main style={shell}>
    ...
    <div style={header}><div><small style={eyebrow}>DELIVERY OPERATIONS</small><h1 style={h1}>{selected.name}</h1><p style={muted}>Prepared for {selected.client_name}{selected.client_email ? ` · ${selected.client_email}` : ''}</p></div><a style={link} href={`/u/••••••••••••••••`} aria-disabled="true"><ShieldCheck size={14}/> Private client link</a></div>
    <section style={grid}>
...
const shell: React.CSSProperties = { minHeight: '100vh', padding: '42px clamp(18px, 4vw, 64px)', background: '#08070c', color: '#f7f5fa', fontFamily: 'Inter, system-ui, sans-serif' }
const header: React.CSSProperties = { maxWidth: 1100, margin: '24px auto 30px', display: 'flex', justifyContent: 'space-between', gap: 24, alignItems: 'flex-start' }
const h1: React.CSSProperties = { margin: '6px 0', fontSize: 'clamp(30px, 5vw, 54px)', letterSpacing: '-.04em' }
const muted: React.CSSProperties = { color: '#9992a6', lineHeight: 1.6, margin: '6px 0' }
const eyebrow: React.CSSProperties = { fontSize: 10, letterSpacing: '.15em', fontWeight: 700, color: '#a992c9' }
const back: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 7, color: '#c9b6e8', textDecoration: 'none', background: 'none', border: 0, padding: 0, cursor: 'pointer' }
const link: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 7, color: '#c9b6e8', fontSize: 12, textDecoration: 'none' }
const grid: React.CSSProperties = { maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0,2fr) minmax(260px,1fr)', gap: 14 }
const card: React.CSSProperties = { border: '1px solid rgba(118,80,173,.22)', borderRadius: 18, padding: 22, background: 'rgba(20,16,28,.82)' }
const label: React.CSSProperties = { display: 'block', margin: '20px 0 7px', fontSize: 12, color: '#c7c0d1' }
const input: React.CSSProperties = { width: '100%', minHeight: 44, boxSizing: 'border-box', padding: '10px 12px', borderRadius: 10, border: '1px solid rgba(153,146,166,.22)', background: '#0e0b13', color: '#f7f5fa', font: 'inherit' }
const primary: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 7, minHeight: 44, padding: '0 16px', borderRadius: 10, border: 0, background: '#7650ad', color: '#fff', fontWeight: 700, cursor: 'pointer' }
const row: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 12, marginTop: 16, flexWrap: 'wrap' }
const success: React.CSSProperties = { display: 'inline-flex', gap: 5, alignItems: 'center', color: '#bfe8c7', fontSize: 12 }
const errorText: React.CSSProperties = { color: '#efaaaa', fontSize: 12 }
const metric: React.CSSProperties = { display: 'flex', justifyContent: 'space-between', gap: 20, padding: '14px 0', borderBottom: '1px solid rgba(153,146,166,.12)', fontSize: 13 }
const activityRow: React.CSSProperties = { display: 'flex', justifyContent: 'space-between', gap: 15, padding: '12px 0', borderBottom: '1px solid rgba(153,146,166,.1)', fontSize: 13 }
const filters: React.CSSProperties = { maxWidth: 1100, margin: '0 auto 14px', display: 'grid', gridTemplateColumns: '1fr 220px', gap: 10 }
const list: React.CSSProperties = { maxWidth: 1100, margin: '0 auto', border: '1px solid rgba(118,80,173,.22)', borderRadius: 18, overflow: 'hidden', background: 'rgba(20,16,28,.72)' }
const projectRow: React.CSSProperties = { width: '100%', display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 15, alignItems: 'center', textAlign: 'left', padding: '18px 20px', border: 0, borderBottom: '1px solid rgba(153,146,166,.1)', background: 'transparent', color: '#f7f5fa', cursor: 'pointer' }
const state: React.CSSProperties = { fontSize: 11, letterSpacing: '.08em', color: '#bca8d8' }
