...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!body?.uploadId) return NextResponse.json({ error: 'Upload ID is required.' }, { status: 400 })
  ...
  if (upload.session_url) {
    ...
    if ((status.status === 200 || status.status === 201) && status.data?.id) {
      ...
      if (file && !file.trashed && Number(file.size || 0) === Number(upload.size_bytes)) {
        return NextResponse.json({ error: 'Upload completed in Google Drive. Refresh the project instead of cancelling it.' }, { status: 409 })
      }
  ...
  await supabase('audit_events', {
    method: 'POST',
    headers: { Prefer: 'return=minimal' },
    body: JSON.stringify({ project_id: project.id, event_type: 'upload.failed', file_name: upload.name, metadata: { uploadId: upload.id } }),
  })
