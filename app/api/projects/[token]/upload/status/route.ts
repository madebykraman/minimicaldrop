...
export async function GET(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const limited = rateLimit(request, 'upload-status', 120, token)
  if (limited) return limited
  const project = await getProjectByToken(token)
  if (!project) return NextResponse.json({ error: 'Upload space is unavailable or expired.' }, { status: 404 })

  const uploadId = new URL(request.url).searchParams.get('uploadId') || ''
  if (!/^[0-9a-f-]{36}$/i.test(uploadId)) return NextResponse.json({ error: 'Upload ID is required.' }, { status: 400 })
  ...
  if (upload.status === 'complete' && upload.drive_file_id) return NextResponse.json({ complete: true, uploadedBytes: Number(upload.size_bytes), driveFileId: upload.drive_file_id })
  ...
  if ((result.status === 200 || result.status === 201) && result.data?.id) {
    ...
    if (file && !file.trashed && Number(file.size || 0) === Number(upload.size_bytes)) return NextResponse.json({ complete: true, uploadedBytes: Number(upload.size_bytes), driveFileId: result.data.id })
  }
  ...
  if (result.status === 404 || result.status === 410) return NextResponse.json({ error: 'Google upload session expired.' }, { status: 410 })
