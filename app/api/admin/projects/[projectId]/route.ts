...
export async function GET(_request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const projects = await supabase<Array<{
    id: string; name: string; client_name: string; client_email: string | null; drive_account_id: string; drive_folder_id: string
    storage_limit_bytes: number | null; expires_at: string; disabled_at: string | null; created_at: string
  ...
  return NextResponse.json({
    ...
    storage: { usedBytes, pendingBytes, limitBytes, availableBytes: limitBytes == null ? null : Math.max(0, Number(limitBytes) - usedBytes - pendingBytes) },
    clientAccess: { path: '/u/••••••••••••••••', tokenStoredAsHash: true, active: !project.disabled_at && new Date(project.expires_at).getTime() > Date.now() },
    activity,
  })
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const existing = await supabase<Array<{
    id: string; name: string; client_name: string; client_email: string | null; storage_limit_bytes: number | null
    expires_at: string; disabled_at: string | null; drive_account_id: string; drive_folder_id: string
  }>>(`projects?id=eq.${encodeURIComponent(projectId)}&select=id,name,client_name,client_email,storage_limit_bytes,expires_at,disabled_at,drive_account_id,drive_folder_id&limit=1`)
  const project = existing[0]
  if (!project) return NextResponse.json({ error: 'Project not found.' }, { status: 404 })

  const body = await request.json().catch(() => null) as {
    name?: string; clientName?: string; clientEmail?: string | null; expiresAt?: string
    storageLimitBytes?: number | null; disabled?: boolean; regenerateToken?: boolean
  } | null
  ...
  if (body?.name !== undefined) {
    ...
    if (!name) return NextResponse.json({ error: 'Invalid project name.' }, { status: 400 })
    nextName = name
    patch.name = name
  }
  if (body?.clientName !== undefined) {
    const clientName = cleanText(body.clientName, 160)
    if (!clientName) return NextResponse.json({ error: 'Invalid client name.' }, { status: 400 })
    patch.client_name = clientName
  }
  if (body?.clientEmail !== undefined) {
    const email = body.clientEmail?.trim() || null
    if (email && (email.length > 320 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))) return NextResponse.json({ error: 'Invalid client email.' }, { status: 400 })
    patch.client_email = email
  }
  ...
  if (body?.disabled !== undefined) {
    ...
    audit.push({ event_type: body.disabled ? 'project.disabled' : 'project.enabled' })
  }
  ...
  if (body?.regenerateToken) {
    ...
    audit.push({ event_type: 'project.token_regenerated' })
  }
  ...
  if (body?.expiresAt !== undefined || body?.storageLimitBytes !== undefined || body?.clientName !== undefined || body?.clientEmail !== undefined) {
    audit.push({ event_type: 'project.updated', metadata: { fields: Object.keys(patch).filter(field => field !== 'access_token_hash' && field !== 'disabled_at') } })
  }
  for (const event of audit) {
    await supabase('audit_events', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ project_id: projectId, event_type: event.event_type, metadata: event.metadata || null }) })
  }
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  if (!(await isAdmin())) return NextResponse.json({ error: 'Unauthorized.' }, { status: 401 })
  const { projectId } = await params
  const projects = await supabase<Array<{ id: string; disabled_at: string | null }>>(`projects?id=eq.${encodeURIComponent(projectId)}&select=id,disabled_at&limit=1`)
