...
create table if not exists projects (
  ...
  delivery_status text not null default 'in_progress' check (delivery_status in ('in_progress','ready','ready_for_review','changes_requested','approved','delivered','archived')),
  client_message text,
  created_at timestamptz not null default now()
);
...
create index if not exists projects_delivery_status_idx on projects(delivery_status);
...
create or replace function public.reserve_upload(p_project_id uuid,p_folder_id uuid,p_name text,p_mime_type text,p_size_bytes bigint,p_session_url text)
returns uuid language plpgsql as $$
declare v_limit bigint; v_expires_at timestamptz; v_disabled_at timestamptz; v_used bigint; v_upload_id uuid;
begin
  if p_size_bytes < 0 then raise exception 'Upload size cannot be negative'; end if;
  select storage_limit_bytes,expires_at,disabled_at into v_limit,v_expires_at,v_disabled_at from public.projects where id=p_project_id for update;
  if not found then raise exception 'Project not found'; end if;
  if v_disabled_at is not null or v_expires_at<=now() then raise exception 'Project is unavailable or expired'; end if;
  select coalesce(sum(size_bytes),0) into v_used from public.uploads where project_id=p_project_id and status in ('uploading','complete');
  if v_limit is not null and v_used+p_size_bytes>v_limit then raise exception 'This upload would exceed the project storage limit'; end if;
  insert into public.uploads(project_id,folder_id,name,mime_type,size_bytes,status,session_url,last_activity_at) values(p_project_id,p_folder_id,p_name,p_mime_type,p_size_bytes,'uploading',p_session_url,now()) returning id into v_upload_id;
  return v_upload_id;
end;
$$;
...
begin
  update public.uploads set last_activity_at=now() where id=p_upload_id and project_id=p_project_id and status='uploading';
